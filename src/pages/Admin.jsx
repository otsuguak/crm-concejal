     ...
     {menuAbierto && <div className="fondo-oscuro-menu" onClick={() => setMenuAbierto(false)}></div>}
      ...
      <main className="admin-main">
        ...
        <div className="stats-grid">
          ...
          <StatCard label="En Gestión" val={casos.filter(c=> c.estado && c.estado.toLowerCase() !== 'solucionado').length} col="#f59e0b" />
        ...
        <div className="table-module">
          ...
          <div style={{display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap', backgroundColor: '#f8fafc', padding: '15px', borderRadius: '12px', border: '1px solid #e2e8f0'}}>
            ...
            <select value={filtroEstado} onChange={e=>setFiltroEstado(e.target.value)} className="search-bar" style={{flex: '1 1 150px'}}><option value="">📌 Todos los Estados</option>{perfil.rol === 'admin' && <option value="ABIERTO">⏳ Abiertos</option>}<option value="Escalado">⚙️ Escalados</option><option value="Solucionado">✅ Solucionados</option></select>
...
const closeBtnStyle = {position: 'absolute', top: '20px', right: '25px', border: 'none', background: '#f1f5f9', width: '40px', height: '40px',  borderRadius: '50%',  display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer', fontWeight: 'bold', color: '#64748b', xShadow: '0 2px 8px rgba(0,0,0,0.05)', zIndex: 10, transition: '0.3s'};
...
const badgeStyle = (est) => {
  ...
  return { padding:'6px 14px', borderRadius:'30px', fontSize:'0.7rem', fontWeight:'800', textTransform:'uppercase', letterSpacing:'0.5px', background: (estReal==='solucionado')?'#dcfce7': (estReal==='escalado')?'#fef3c7':'#fee2e2', color: (estReal==='solucionado')?'#166534': (estReal==='escalado')?'#92400e':'#991b1b' };
};
